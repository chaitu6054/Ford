...
def _without_pivot_cache_records() -> Iterator[None]:
    """Load workbooks while skipping pivot-cache record data.

    Some analyst workbooks embed very large pivot caches; openpyxl parses the
    full cache records on every ``load_workbook`` call, which can stall for
    many minutes. The France calculations only need cell values and fills and
    never touch pivot data, so cache definitions are loaded without their
    record payloads while this context is active. The stock behaviour is
    restored on exit. Not thread-safe by design (single-threaded runners).
    """
    try:
        ...
        try:
            ...
            WorkbookParser.pivot_caches = original
    except Exception:
        raise
