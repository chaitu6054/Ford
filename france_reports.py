...
def generate_france_reports(
    *,
    ...
    try:
        ...
        era_path: Path | None = out_dir / f"france_era_{month}.xlsx"
        portefeuille_path_out: Path | None = (
            out_dir / f"france_portefeuille_{month}.xlsx"
        )
        france_pipeline.ecrire_classeur(era_tables, str(era_path))
        if portefeuille_tables:
            france_pipeline.ecrire_classeur(portefeuille_tables, str(portefeuille_path_out))
        else:
            # No portefeuille input (or no R3 tables): saving zero sheets makes
            # openpyxl raise "At least one sheet must be visible", so skip the
            # file instead of writing an empty workbook.
            LOGGER.info(
                "No portefeuille tables: skipping %s",
                f"france_portefeuille_{month}.xlsx",
            )
            portefeuille_path_out = None
        LOGGER.info(
            "France reports generated: %s, %s", era_path.name, portefeuille_path_out
        )
